...
resource "azurerm_vpn_gateway" "hub1vpngw" {
  ...
  depends_on = [
    azurerm_virtual_hub_connection.tofirewallvnet
  ]
}
...
resource "azurerm_vpn_gateway" "hub2vpngw" {
  resource_group_name = azurerm_resource_group.RG.name
  name                = "hub2vpngw"
  location            = azurerm_virtual_hub.vhub2.location
...
resource "azurerm_vpn_site" "onprem2" {
  device_vendor       = "Azure"
  location            = azurerm_resource_group.RG.location
  name                = "onprem2"
  resource_group_name = azurerm_resource_group.RG.name
  virtual_wan_id      = azurerm_virtual_wan.vwan1.id
  link {
    ip_address    = azurerm_public_ip.onprem2vpngw-pip.ip_address
    name          = "onpremlink2"
    provider_name = "Azure"
    speed_in_mbps = 10
    bgp {
      asn             = 65001
      peering_address = azurerm_virtual_network_gateway.onprem2vpngw.bgp_settings[0].peering_addresses[0].default_addresses[0]
    }
  }
}
resource "azurerm_vpn_gateway_connection" "onpremconnection2" {
  internet_security_enabled = true
  name                      = "Connection-onprem2"
  remote_vpn_site_id        = azurerm_vpn_site.onprem2.id
  vpn_gateway_id            = azurerm_vpn_gateway.hub2vpngw.id
  vpn_link {
    bgp_enabled      = true
    name             = "onpremlink2"
    shared_key       = "vpn123"
    vpn_site_link_id = azurerm_vpn_site.onprem2.link[0].id
  }
...
data "azurerm_virtual_hub_route_table" "hub2defaultrt" {
  name                = "defaultRouteTable"
  resource_group_name = azurerm_resource_group.RG.name
  virtual_hub_name    = azurerm_virtual_hub.vhub2.name
}

resource "azurerm_virtual_hub_route_table_route" "route2" {
  route_table_id = data.azurerm_virtual_hub_route_table.hub2defaultrt.id
...
resource "azurerm_public_ip" "spoke3vm-pip" {
  name                = "spoke3vm-pip"
  location            = azurerm_resource_group.RG.location
  resource_group_name = azurerm_resource_group.RG.name
  allocation_method = "Dynamic"
...
resource "azurerm_public_ip" "onprem2vpngw-pip" {
  name                = "onprem2vpngw-pip"
  location            = azurerm_resource_group.RG.location
  resource_group_name = azurerm_resource_group.RG.name
  allocation_method = "Static"
  sku = "Standard"
...
resource "azurerm_public_ip" "onpremvm2-pip" {
  name                = "onpremvm2-pip"
  location            = azurerm_resource_group.RG.location
  resource_group_name = azurerm_resource_group.RG.name
  allocation_method = "Dynamic"
...
resource "azurerm_virtual_network_gateway" "onprem2vpngw" {
  name                = "onprem2VPNGW"
  location            = azurerm_resource_group.RG.location
  resource_group_name = azurerm_resource_group.RG.name
  type     = "Vpn"
  ...
  ip_configuration {
    name                          = "vnetGatewayConfig"
    public_ip_address_id          = azurerm_public_ip.onprem2vpngw-pip.id
    private_ip_address_allocation = "Dynamic"
    subnet_id                     = azurerm_virtual_network.onprem2-vnet.subnet.*.id[1]
...
resource "azurerm_local_network_gateway" "vwanlng" {
  ...
  gateway_address     = data.azurerm_vpn_gateway.hub1pip.bgp_settings[0].instance_0_bgp_peering_address[0].tunnel_ips[0]
  location            = azurerm_resource_group.RG.location
  name                = "vwanlng"
  resource_group_name = azurerm_resource_group.RG.name
  bgp_settings {
    asn = "65515"
    bgp_peering_address = data.azurerm_vpn_gateway.hub1pip.bgp_settings[0].instance_0_bgp_peering_address[0].default_ips[0]
  }
  timeouts {
    create = "2h"
    read = "2h"
...
resource "azurerm_local_network_gateway" "vwanlng2" {
  #address_space       = ["10.0.0.0/16", "10.250.0.0/16"]
  gateway_address     = data.azurerm_vpn_gateway.hub2pip.bgp_settings[0].instance_0_bgp_peering_address[0].tunnel_ips[0]
  location            = azurerm_resource_group.RG.location
  name                = "vwanlng2"
  resource_group_name = azurerm_resource_group.RG.name
  bgp_settings {
    asn = "65515"
    bgp_peering_address = data.azurerm_vpn_gateway.hub2pip.bgp_settings[0].instance_0_bgp_peering_address[0].default_ips[0]
  }
  timeouts {
    create = "2h"
    read = "2h"
...
resource "azurerm_virtual_network_gateway_connection" "to-azure2" {
  local_network_gateway_id   = azurerm_local_network_gateway.vwanlng2.id
  location                   = azurerm_resource_group.RG.location
  name                       = "to-vwan2"
  resource_group_name        = azurerm_resource_group.RG.name
  shared_key                 = "vpn123"
  type                       = "IPsec"
  virtual_network_gateway_id = azurerm_virtual_network_gateway.onprem2vpngw.id
  enable_bgp = true
  timeouts {
    create = "2h"
    read = "2h"
...
resource "azurerm_subnet_route_table_association" "ononpremdefaultsubnet2" {
  subnet_id      = azurerm_virtual_network.onprem2-vnet.subnet.*.id[0]
  route_table_id = azurerm_route_table.RT.id
  timeouts {
    create = "2h"
    read = "2h"
...
resource "azurerm_network_interface" "spoke3vm-nic" {
  location            = azurerm_resource_group.RG.location
  name                = "spoke3vm-nic"
  resource_group_name = azurerm_resource_group.RG.name
  ip_configuration {
    name                          = "ipconfig1"
    private_ip_address_allocation = "Dynamic"
    public_ip_address_id          = azurerm_public_ip.spoke3vm-pip.id
    subnet_id                     = azurerm_virtual_network.spoke3-vnet.subnet.*.id[0]
  }
  timeouts {
    create = "2h"
...
resource "azurerm_network_interface" "onpremvm2-nic" {
  location            = azurerm_resource_group.RG.location
  name                = "onpremvm2-nic"
  resource_group_name = azurerm_resource_group.RG.name
  ip_configuration {
    name                          = "ipconfig1"
    private_ip_address_allocation = "Dynamic"
    public_ip_address_id          = azurerm_public_ip.onpremvm2-pip.id
    subnet_id                     = azurerm_virtual_network.onprem2-vnet.subnet.*.id[0]
  }
  timeouts {
    create = "2h"
...
resource "azurerm_windows_virtual_machine" "spoke3vm" {
  admin_password        = var.E-password
  admin_username        = var.D-username
  location              = azurerm_resource_group.RG.location
  name                  = "spoke3vm"
  network_interface_ids = [azurerm_network_interface.spoke3vm-nic.id]
  resource_group_name   = azurerm_resource_group.RG.name
  size                  = "Standard_B2ms"
  identity {
    type = "SystemAssigned"
...
resource "azurerm_virtual_machine_extension" "killspoke3vmfirewall" {
  auto_upgrade_minor_version = true
  name                       = "killspokevmfirewall"
  publisher                  = "Microsoft.Compute"
  type                       = "CustomScriptExtension"
  type_handler_version       = "1.10"
  virtual_machine_id         = azurerm_windows_virtual_machine.spoke3vm.id
  settings = <<SETTINGS
    {
      "commandToExecute": "powershell -command \"Set-NetFirewallProfile -Enabled False\""
    }
...
resource "azurerm_windows_virtual_machine" "onpremvm2" {
  admin_password        = var.E-password
  admin_username        = var.D-username
  location              = azurerm_resource_group.RG.location
  name                  = "onpremvm2"
  network_interface_ids = [azurerm_network_interface.onpremvm2-nic.id]
  resource_group_name   = azurerm_resource_group.RG.name
  size                  = "Standard_B2ms"
  identity {
    type = "SystemAssigned"
...
resource "azurerm_virtual_machine_extension" "killonpremvm2firewall" {
  auto_upgrade_minor_version = true
  name                       = "killonpremvmfirewall"
  publisher                  = "Microsoft.Compute"
  type                       = "CustomScriptExtension"
  type_handler_version       = "1.10"
  virtual_machine_id         = azurerm_windows_virtual_machine.onpremvm2.id
  settings = <<SETTINGS
    {
      "commandToExecute": "powershell -command \"Set-NetFirewallProfile -Enabled False\""
    }
